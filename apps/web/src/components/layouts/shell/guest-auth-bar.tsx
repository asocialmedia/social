...
export const GuestAuthBar: React.FC = () => {
  ...
  // Same scroll signal as the bottom nav (same event, threshold, and rule),
  // so the banner slides away and back in lockstep with it instead of
  // hovering over the content while the nav is gone.
  ...
  return (
    ...
    >
      {/* Slides fully below the fold on scroll-down and back on scroll-up,
          in lockstep with the bottom nav. Visible by default - motion reacts
          to scrolling, it never gates the banner's existence. */}
      <motion.div
        animate={
          reduceMotion
            ? { opacity: hidden ? 0 : 1 }
            : { opacity: hidden ? 0 : 1, y: hidden ? "110%" : "0%" }
        }
        initial={false}
