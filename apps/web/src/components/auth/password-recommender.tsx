...
export function PasswordRecommender({
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Password recommendation logic requires multiple validation and transformation steps
  const generateRecommendation = (inputPassword: string): string => {
    if (!password) {
      return "";
    }
    ...
    let failedRequirements = requirements.filter(
      (req) => !req.validator(inputPassword)
    );
    ...
    for (const req of failedRequirements) {
      ...
      failedRequirements = requirements.filter(
        (requirement) => !requirement.validator(recommendation)
      );
    }
